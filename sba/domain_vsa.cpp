...
/* ---------------------------------- BaseStride ---------------------------------- */
/* 			            (VSA's Domain) 			            */
/* NOTE: Comments '//' highlights differences between SJA's Domain and VSA's Domain */
...
void BaseStride::strip() {
   /* 1st satisfies */
   if (!top() && !bot() && !empty() && !pc() &&
   !(x != nullptr && x->x != nullptr && (x->x->nmem() || x->x->mem()))) {
      if (next != nullptr) delete next;
      next = nullptr;
      return;
   }
   /* some other satisfies */
   auto curr = next;
   while (curr != nullptr) {
      if (!curr->top() && !curr->bot() && !curr->empty() && !curr->pc() &&
      !(curr->x != nullptr && curr->x->x != nullptr && (curr->x->x->nmem() || curr->x->x->mem()))) {
         unit_assign(*curr);
         if (next != nullptr) delete next;
         next = nullptr;
         return;
...
bool BaseStride::unit_equal(const BaseStride& object) const {
   return ((t == object.t || (t == T::NMEM && object.t == T::CONST)
       ...
       && (x != nullptr && object.x != nullptr &&
          (object.x->t == T::TOP || object.x->t == T::DYNAMIC) &&
          (x->t == T::TOP || x->t == T::DYNAMIC) &&
          x->range == object.x->range && x->range != Range::FULL));
}
...
void BaseStride::unit_mem(const BaseStride& object, uint8_t width) {
   if (object.t == T::DYNAMIC || object.t == T::TOP || object.t == T::MEM) {
      unit_type(T::TOP);
      #if ENABLE_SUPPORT_CONSTRAINT
         range = Range::FULL;
      #endif
      ...
      // changed!
      else if (object.x==nullptr || object.x->top() || object.x->dynamic()) {
         t = T::MEM;
         b = object.b;
         s = object.s;
         w = width;
...
void BaseStride::abs_union(const BaseStride& object) {
   ...
   // changed (added):
   if ((top() && dynamic())) {
      if (object.top() || object.dynamic()) {
         #if ENABLE_SUPPORT_CONSTRAINT
            range = range | object.range;
         #endif
      }
      else if (object.cst() || (object.nmem() && object.s == 0)) {
         #if ENABLE_SUPPORT_CONSTRAINT
            range = range | Range(object.b, object.b);
         #endif
      }
      else
         unit_type(T::TOP);
   }
   else if (cst() || (nmem() && s == 0)) {
      if (object.top() || object.dynamic()) {
         t = T::TOP;
         #if ENABLE_SUPPORT_CONSTRAINT
            range = range | object.range;
   ...
   else if (t == object.t && (nmem() || mem())) {
      if (!(b == object.b && s == object.s && x->unit_equal(*object.x)))
         unit_type(T::TOP);
   }
      ...
      for (const BaseStride* Y = &object; Y != nullptr; Y = Y->next) {
         ...
         && (X->t != T::MEM || Y->t != T::MEM)) {
            if (X->t == T::NMEM || X->t == T::CONST) {
               ...
               /* (MEM,3,4,x') - (NMEM,5,0,null) = (NMEM,-5 */
               if (Y->s == 0) {
                  // Z->t = T::NMEM;
                  // Z->b = -Y->b;
                  // Z->s = 1;
