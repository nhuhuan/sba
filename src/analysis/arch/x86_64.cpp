...
#include <analysis/core/framework.h>
#include <analysis/cfg/intra/insn.h>
#include <analysis/arch/x86_64.h>
#include <array>
...
void x86_64::load(Object& info, const string& file) {
   string s;
   string cmd;
...
uint64_t x86_64::read(const Object& info, int64_t offset, uint8_t width) {
...
bool x86_64::code_ptr(const Object& info, IMM ptr) {
   if (!info.insns->empty())
...
unordered_set<IMM> x86_64::stored_cptrs(const Object& info, uint8_t size) {
   unordered_set<IMM> cptrs;
   for (IMM offset = 0; offset < (IMM)(info.raw_bytes.size()-size+1); ++offset) {
      auto val = x86_64::read(info, offset, size);
      if (x86_64::code_ptr(info, val))
         cptrs.insert(val);
   }
   return cptrs;
}
...
unordered_set<IMM> x86_64::definite_fptrs(const Object& info, const string& file) {
...
unordered_set<IMM> x86_64::noreturn_fptrs(const string& file) {
   ...
   for (auto [call, sym]: x86_64::import_symbols(file))
...
unordered_set<IMM> x86_64::noreturn_calls(const string& file) {
   unordered_set<IMM> res;
   auto noret = noreturn_fptrs(file);
   for (auto [offset, target]: x86_64::call_insns(file))
...
void x86_64::disassemble(const string& file, const string& f_asm, const
string& f_raw) {
   /* disassembly */
   string s;
...
vector<pair<IMM,IMM>> x86_64::import_symbols(const string& file) {
...
vector<pair<IMM,IMM>> x86_64::call_insns(const string& file) {
   auto cmd = string("objdump --prefix-addresses --no-show-raw-insn -M intel -d ")
...
uint8_t x86_64::prolog(const vector<uint8_t>& raw_insn) {
